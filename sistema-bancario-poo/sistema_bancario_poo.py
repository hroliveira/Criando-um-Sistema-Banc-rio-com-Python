        ...
        self._transacoes.append(
            {
                ...
                "data": datetime.now().strftime("%d-%m-%Y %H:%M:%S5"),
            }
        )
...
def exibir_extrato(clientes):
    ...
    print("\n================ EXTRATO ================")
    print(f"Conta: {conta.numero}\tTitular: {cliente.nome}")
    transacoes = conta.historico.transacoes

    #extrato = ""
    if not transacoes:
        print( "\nOps, parece que não há nada para ver aqui! Nenhuma movimentação foi registrada.")
    else:
        print("\n{:<25} {:<25} {:<25}".format("TIPO", "VALOR", "HORA"))
        for transacao in transacoes:
            tipo = transacao['tipo']
            valor = f"R$ {transacao['valor']:.2f}" if tipo == 'Deposito' else f"- R$ {transacao['valor']:.2f}"
            hora = transacao['data']
            print("\n{:<25} {:<25} {:<25}".format(tipo, valor, hora))
